...
class OrderRequests extends Component {
    ...
    handleSendToDeliveredBtn(userUid, orderId) {
        ...
        firebase.firestore().collection('users').doc(restaurantUid).collection('orderRequest').doc(orderId).update({
            ...
            console.log("First Successfully send to IN PROGRESS")
            firebase.firestore().collection('users').doc(userUid).collection('myOrder').doc(orderId).update({
                status: "DELIVERED",
                delivery_timestamp: firebase.firestore.FieldValue.serverTimestamp(),
            }).then(() => {
                console.log("Second Successfully send to IN PROGRESS")
            })
        })
    }
                    ...
                    return (
                        <div className="container border-bottom pb-2 px-lg-0 px-md-0 mb-4" key={orderRequest[val].id}>
                            ...
                            <div className="p-3" style={{ wordWrap: "break-word" }}>
                                <p className="text-secondary">Address: {orderRequest[val].address}</p>
                            </div>
                            {
                                Object.keys(orderRequest[val].itemsList).map((val2) => {
                                    ...
                                    return (
                                        <div className="row mb-3" key={val2}>
                                            ...
                                            <div className="col-lg-5 col-md-3 col-sm-12 px-0">
                                                <h6 className="">{orderRequest[val].itemsList[val2].itemTitle}</h6>
                                            </div>
                                            <div className="col-lg-2 col-md-3 col-sm-12 px-0">
                                                <h6 className="">x{orderRequest[val].itemsList[val2].quantity}</h6>
                                            </div>
                            ...
                            <p><b className="mr-4">Order Time:</b>{orderRequest[val].order_timestamp.toDate().toLocaleString()}</p>
                        </div>
                    )
                }
            })
                ...
                if (orderRequest[val].status === "DELIVERED") {
                    ...
                    return (
                        <div className="container border-bottom pb-2 px-lg-0 px-md-0 mb-4" key={orderRequest[val].id}>
                            ...
                            <div className="col-lg-6 col-md-6 col-12">
                                <div>
                                    <b className="mr-4">Order Time:</b>
                                    {orderRequest[val].order_timestamp.toDate().toLocaleString()}
                                </div>
                                <div>
                                    <b className="mr-4">Delivery Time:</b>
                                    {orderRequest[val].delivery_timestamp.toDate().toLocaleString()}
                                </div>
                            </div>
                        </div>
                    )
                                    ...
                                    {tab1Content &&
                                        < div className="row pending-order-section">
                                            <div className="col-12 bg-white p-4">
                                                {this._renderPendingOrderRequest() || <EmptyContainer/>}
                                            </div>
                                        </div>
                                    }
                                    {tab2Content && <div className="row inProgress-order-section">
                                        <div className="col-12 bg-white p-4">
                                            {this._renderInProgressOrderRequest()|| <EmptyContainer/>}
                                        </div>
                                    </div>
                                    }
                                    {tab3Content && <div className="row delivered-order-section">
                                        <div className="col-12 bg-white p-4">
                                            {this._renderDeliveredOrderRequest()|| <EmptyContainer/>}
                                        </div>
                                    </div>
                                    }
                                </div>
